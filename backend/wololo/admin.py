...
class MyUserAdmin(UserAdmin):
    ...
    list_display = ['id', 'points', 'username', 'number_of_villages', 'email',
                    'is_region_selected', 'is_unviewed_reports_exists', 'clan_id', ]

    fieldsets = UserAdmin.fieldsets + (
        (None, {'fields': ('is_region_selected',
                           'is_unviewed_reports_exists', 'number_of_villages', 'points')}),
    )
...
class VillagesAdminForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        instance = super(VillagesAdminForm, self).save(commit=False)
        if commit:
...
class TroopMovementsAdmin(admin.ModelAdmin):
    ...
    list_display = ['task_id', 'movement_type',
